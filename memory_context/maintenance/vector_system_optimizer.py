...
DEFAULT_CONFIG = {
    ...
    "auto_cleanup_orphans": False,   # 🔒 安全修复：默认禁用自动清理孤立向量
...
class VectorSystemOptimizer:
        ...
        try:
            ...
            for table in tables:
                try:
                    ...
                    # 注意：表名不能参数化，但这里的表名是硬编码的白名单，安全
                    if table in ['l1_records', 'l1_vec', 'l0_conversations', 'l0_vec', 'l1_fts']:
    ...
    def optimize(self):
        """完整优化"""
        # 🔒 安全修复：检查是否启用自动优化
        auto_enabled = (self.config.get('auto_vacuum', False) or 
                       self.config.get('auto_reindex', False) or 
                       self.config.get('auto_cleanup_orphans', False))
        
        if not auto_enabled:
            print("⚠️ 所有自动优化已禁用")
            print("请手动运行以下命令：")
            print("  - 清理孤立向量: python3 vector_system_optimizer.py cleanup")
            print("  - VACUUM: python3 vector_system_optimizer.py vacuum")
            print("  - 重建索引: python3 vector_system_optimizer.py reindex")
            print("  - 完整优化: python3 vector_system_optimizer.py run")
            return
        ...
        # 3. 清理孤立向量
        if self.config.get('auto_cleanup_orphans', False) and orphans['total_orphans'] > self.config['orphan_threshold']:
            self.cleanup_orphans()
        
        # 4. VACUUM
        if self.config.get('auto_vacuum', False) and stats.get('db_size_mb', 0) > self.config['max_db_size_mb']:
            self.vacuum()
...
def main():
    ...
    if cmd == "status":
        stats = optimizer.get_db_stats()
        orphans = optimizer.check_orphans()
        print("=" * 60)
        print("向量系统状态")
        print("=" * 60)
        print(f"数据库大小: {stats.get('db_size_mb', 0):.2f} MB")
        print(f"L1 记录: {stats.get('l1_records', 0)} 条")
        print(f"L0 记录: {stats.get('l0_conversations', 0)} 条")
        print(f"孤立向量: L1 {orphans['l1_orphans']} 条, L0 {orphans['l0_orphans']} 条")
        print("\n配置:")
        for k, v in optimizer.config.items():
            print(f"  {k}: {v}")
    elif cmd == "cleanup":
        optimizer.cleanup_orphans()
    elif cmd == "vacuum":
        optimizer.vacuum()
    elif cmd == "reindex":
        optimizer.reindex_fts()
    elif cmd == "run":
        optimizer.optimize()
    else:
        print("用法: vector_system_optimizer.py [status|cleanup|vacuum|reindex|run]")
