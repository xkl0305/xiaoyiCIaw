...
接收用户的自然语言查询请求，分析用户意图，识别基金理财相关查询类型：
- 基金业绩/持仓查询
- 基金风险/评级查询
- 基金经理/基金公司查询
...
| `X-Claw-Skill-Id` | `hithink-fund-query`（与 skill name 一致） |
...
headers = {
    ...
    "X-Claw-Skill-Id": "hithink-fund-query",
...
```python
for item in datas:
    fund_code = item.get("基金代码")       # 如 "000001"
    fund_name = item.get("基金名称")       # 如 "华夏成长混合"
    # 其他字段根据查询类型而定，如业绩指标、风险评级、基金经理等
```
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含基金代码、基金名称、基金类型、基金经理等字段 |
...
{
  "datas": [
    {"基金代码": "000001", "基金名称": "华夏成长混合", "基金类型": "混合型", "基金经理": "张三", "成立以来收益": "120.5%"}
  ],
  "code_count": 1256,
  "chunks_info": {
    "query": "业绩最好的基金",
    "parsed_conditions": ["业绩最好的", "基金"]
  }
}
```
...
```bash
# 查询业绩最好的基金
python3 scripts/cli.py --query "业绩最好的基金"

# 查询基金持仓明细
python3 scripts/cli.py --query "基金持仓明细"
...
python3 scripts/cli.py --query "业绩最好的基金" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "业绩最好的基金" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-fund-query" \
...
} -Body '{"query":"业绩最好的基金","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
