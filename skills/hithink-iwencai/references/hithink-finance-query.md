...
本技能提供财务数据查询能力，支持：
- **盈利指标**：营业收入、净利润、毛利率、净利率等
- **回报指标**：ROE、ROA、净资产收益率等
- **偿债能力**：负债率、资产负债结构等
- **现金流指标**：现金流、经营性现金流、投资性现金流等
- **估值指标**：市盈率、市净率、市销率等
...
接收用户的自然语言查询请求，分析用户意图，识别财务数据相关查询类型：
- 盈利指标查询
- 回报/偿债能力查询
- 现金流/估值指标查询
...
for item in datas:
    ...
    # 其他字段根据查询类型而定，如营业收入、净利润、ROE、负债率等
```
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含股票代码、股票简称、营业收入、净利润等字段 |
...
{
  "datas": [
    {"股票代码": "300033.SZ", "股票简称": "同花顺", "营业收入": "35.23亿", "净利润": "12.56亿"}
  ],
  "code_count": 5236,
  "chunks_info": {
    "query": "同花顺营业收入 净利润",
    "parsed_conditions": ["同花顺", "营业收入", "净利润"]
  }
}
```
...
python3 scripts/cli.py --query "同花顺营业收入" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "同花顺营业收入" --timeout 60
```
...
} -Body '{"query":"同花顺营业收入 净利润","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
