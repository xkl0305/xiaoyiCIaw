---
name: hithink-futures-selector
description: 通过行情、波动率、产销、会员持仓、会员榜单、行权等多条件组合筛选期货期权。返回符合条件的相关期货期权数据。当用户询问期货筛选、期权筛选问题时，必须使用此技能。
...
**改写规则：**
- 保留用户核心意图（如：原油期货、多头持仓等）
- 将口语化表达转为标准金融术语（如"帮我选螺纹钢" → "螺纹钢期货有哪些"）
- 适当简化过于复杂的复合条件
- 改写后需保持原意不变
...
| `X-Claw-Skill-Id` | `hithink-futures-selector`（与 skill name 一致） |
...
headers = {
    ...
    "X-Claw-Skill-Id": "hithink-futures-selector",
...
# 解析返回数据
datas = result.get("datas", [])           # 当前页期货期权列表
code_count = result.get("code_count", 0)  # 符合条件的总期货期权数
chunks_info = result.get("chunks_info", {})  # 查询字句信息

# 分页提示：如果 code_count > len(datas)，通过增加 page 参数翻页
```

**注意：** 默认返回 10 条数据，但符合条件的期货期权总数可能更多，需关注 `code_count` 字段并通过分页获取全部数据。
...
```python
for item in datas:
    futures_code = item.get("期货代码")       # 如 "AU2506.SHF"
    futures_name = item.get("期货名称")       # 如 "黄金期货"
    # 其他字段根据查询类型而定
```
...
- 如果数据完整：直接返回格式化后的结果且保证选期货期权表格正确解析为表格展示
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含期货代码、期货名称、涨跌幅等字段 |
| code_count | INT | 符合查询条件的总期货期权数量（注意：可能大于当前返回的 datas 条数） |
...
{
  "datas": [
    {"期货代码": "AU2506.SHF", "期货名称": "黄金期货", "涨跌幅": 1.25},
    {"期货代码": "CU2506.SHF", "期货名称": "铜期货", "涨跌幅": 0.85}
  ],
  "code_count": 30,
  "chunks_info": {
    "query": "黄金期货",
    "parsed_conditions": ["期货品种=黄金"]
  }
}
```
...
- `code_count` 表示符合条件的总期货期权数，可能远大于 `datas` 的长度
- 当 `code_count > len(datas)` 时，需要通过 `page` 参数翻页获取更多数据
- 返回的表格数据需要解析 `datas` 数组中的对象字段，如 `期货代码`、`期货名称`、`涨跌幅` 等
...
```bash
# 直接查询（默认返回 10 条）
python3 scripts/cli.py --query "原油期货有哪些？"

# 指定分页参数
python3 scripts/cli.py --query "黄金期货行情" --page "1" --limit "20"

# 指定 API 密钥
python3 scripts/cli.py --query "多头持仓的期货" --api-key "your-key"

# 重试请求（放宽条件后使用 retry 标记）
python3 scripts/cli.py --query "原油期货有哪些？" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "原油期货有哪些？" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-futures-selector" \
...
} -Body '{"query":"黄金期货","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
