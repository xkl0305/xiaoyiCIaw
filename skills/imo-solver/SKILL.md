...
3. **等待结果（poll + sleep 循环）**：
   ...
   **超时判断**：
   - 简单题目：累计等待超过 20 分钟仍未完成，停止循环，向用户报告超时
   - IMO 题目：累计等待超过 60 分钟仍未完成，停止循环，向用户报告超时
...
- **stdout 格式**：脚本运行中输出 `=== PROGRESS === <内容>` 进度行（flush 实时刷新），结束时输出 `=== RESULT ===` 后紧跟完整 JSON。poll 返回中按这两个标记区分进度和最终结果。
- **轮询纪律**：poll 返回 `=== PROGRESS ===` 时简短转述进度给用户，返回 `=== RESULT ===` 时转述完整结果；返回 running 时保持沉默不输出。不读日志、不调用其他工具，只做 `poll → sleep` 循环。
