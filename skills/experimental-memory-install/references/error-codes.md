...
|---|---|---|---|
| `0` | success | 成功 | — |
| `10` | argument / precondition error (mutually exclusive flags, missing local tarball, bad mode, remote flag on offline skill) | 参数或前置条件错误（互斥标志、本地包缺失、未知 mode、离线 skill 收到远端标志） | For `/experimental-memory-install`, pre-stage a matching tarball in `$CELIA_TARBALL_DIR`. Do not pass remote flags to the offline celiaclaw skill. |
| `20` | network failure (artifact mirror clone, asset download) | 网络失败（镜像 clone 或资产下载） | Check connectivity to `gitcode.com`. The mirror cache lives at `~/.openclaw/extensions/celia_memory/_artifacts-mirror/`; `rm -rf` it to force a re-clone. |
...
| `70` | lock contention (concurrent install) or insufficient disk | 锁竞争或磁盘空间不足 | Concurrent install: wait or check `~/.openclaw/extensions/celia_memory/.lock` for stale processes. Disk: free up the partition holding `~/.openclaw/extensions/celia_memory/` (need ~3× tarball size). |
...
happened; check `~/.openclaw/logs/celia_memory/` and the partial state under
`~/.openclaw/extensions/celia_memory/install/<v>-tmp/` (if present) to assess.
