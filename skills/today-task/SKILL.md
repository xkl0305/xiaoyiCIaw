...
### 2. 实际端点
数据发送到华为云端点，URL由`SERVICE_URL`配置和固定后缀组成：
```
{SERVICE_URL}/celia-claw/v1/rest-api/skill/execute
```
...
### **配置说明**：推送URL由`SERVICE_URL`配置和固定后缀`/celia-claw/v1/rest-api/skill/execute`组成**
...
- **端点来源**：`SERVICE_URL`配置 + 固定后缀`/celia-claw/v1/rest-api/skill/execute`
