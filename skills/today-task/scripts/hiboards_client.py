...
class HiboardsClient:
    ...
    BASE_URL_SUFFIX = "/celia-claw/v1/rest-api/skill/execute"

    def __init__(self, config):
        """初始化客户端"""
        self.config = config
        self.base_url_suffix = self.BASE_URL_SUFFIX
        ...
        logger.info(f"目标URL后缀: {self.base_url_suffix}")
    ...
    def push(self, push_data: Dict[str, Any]) -> Tuple[bool, Any]:
        ...
        if not check_result:
            return False, '缺少必填配置，PERSONAL-API-KEY或PERSONAL-UID或SERVICE_URL配置'
        
        # 构建完整URL：SERVICE_URL + 固定后缀
        api_url_prefix = config.get('SERVICE_URL', 'https://celia-claw-drcn.ai.dbankcloud.cn')
        url = api_url_prefix + self.BASE_URL_SUFFIX
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            # 尝试访问状态端点（如果存在）
            status_url = self.base_url_suffix.replace('/rest-api/skill/execute', '/status')
