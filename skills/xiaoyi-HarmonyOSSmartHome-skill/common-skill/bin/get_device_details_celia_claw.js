import {
  generateTraceId,
  maskSecret, hagSkillServicePostWithPathParams
} from '../../utils/hag-connect/utils.js';

export async function getDeviceDetailCeliaClaw(devId, verbose = false) {
  ...
  let response;
  try {
    response = await hagSkillServicePostWithPathParams("getDevSnapshot", body, verbose);
  } catch (error) {
    const errorResult = {
      success: false,
      error: {
