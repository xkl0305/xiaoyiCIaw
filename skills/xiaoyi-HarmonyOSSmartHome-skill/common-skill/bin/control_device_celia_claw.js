import { hagSkillServicePost, generateTraceId, maskSecret } from '../../utils/hag-connect/utils.js';

export async function controlDeviceCeliaClaw(devId, prodId, devType, operation, sid, data, verbose = false) {
  ...
  if (!devType || typeof devType !== 'string') {
    const error = {
      success: false,
      error: {
        code: 'INVALID_PARAM',
        message: 'deviceType 不能为空且必须是字符串',
        field: 'deviceType'
      },
      traceId
    };
  ...
  let response;
  try {
    response = await hagSkillServicePost("devCommand", body, verbose);
  } catch (error) {
    const errorResult = {
      success: false,
      error: {
