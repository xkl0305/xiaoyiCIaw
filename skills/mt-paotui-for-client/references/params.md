...
> ⚠️ **严格查表规则**：`goodTypes` 数值与 `goodTypeNames` 名称必须**整行一起取**，禁止单独推断（如"餐饮"绝不是 `[1]`，"鲜花"绝不是 `[2]`）。每次都必须找到用户描述对应的那一整行，同时取出该行的 `goodTypes` 和 `goodTypeNames`，缺一不可。

| 用户描述关键词 | goodTypes（数值） | goodTypeNames（名称） | goodsWeight 默认值 |
|---|---|---|---|
