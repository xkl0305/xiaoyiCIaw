...
麦当劳 MCP 服务的错误处理规范，所有业务域通用。调用方式与 Token 配置见 `references/tool-calling-strategy.md`。

## HTTP 错误

| HTTP 状态码 | 含义 | 处理方式 |
|------------|------|---------|
| 401 | Token 无效或已过期 | 检测是否调用过huawei_id_tool("117797261", "mcd-skills")，`如果未调用，则调用一次` huawei_id_tool("117797261", "mcd-skills")` 刷新 Token |
...
- 若返回体不是预期 JSON 或缺少 `result`，先检查网络、URL、`Authorization` 是否与 `tool-calling-strategy.md` 一致
- `tools/call` 返回的 `error` 对象：将可读信息转述给用户，技术细节不展开

## 管道过滤异常（`filter_response.py`）

当 `call_tool.sh --extract` 的输出通过管道传给 `filter_response.py` 时，可能出现以下异常：
...
1. `filter_response.py` 的错误输出到 stderr，调用时用 `2>&1` 合并才能在对话上下文中看到
2. 遇到参数错误（如 `检查必填参数`）时，查阅 `scripts/cache/` 中的工具 schema 确认必填字段，修正后重试
