...
- 单个地址 → **自动选择**，告知用户"使用你的地址：XXX"
- 多个地址 → 优先选择用户画像中 `addresses.lastUsed` 对应的地址；若无画像则让用户选择
...
0. 检查 `117797261_login_token` 是否有效；过期则调用 `huawei_id_tool("117797261","mcd-skills")` 刷新，仅调用一次，不可重复调用
