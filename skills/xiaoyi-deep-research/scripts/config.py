...
def load_dotenv():
    ...
    path = str(Path.home() / ".openclaw" / ".xiaoyiruntime")
    if not Path(path).is_file():
        return
    with open(path, encoding="utf-8") as f:
        for line in f:
...
class Config:
    ...
    def load() -> "Config":
        ...
        if not cfg.personal_uid:
            raise ValueError("用户id未设置")
        ...
        cfg.sn = (os.getenv("TASK_ID", "") + "1").replace("&", "_")
        cfg.sessionId = os.getenv("TASK_ID", "").replace("&", "_")
