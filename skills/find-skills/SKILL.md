---
name: find-skills
description: 帮助用户查询或安装 agent skill。触发条件包括：1）用户明确要求查找、搜索、推荐、安装、重装、重载某个 Skill 时（例如：“查找有没有做 XX 的 skill”、“帮我安装一个 XX”），应使用本 skill；2）当用户表达对当前任务完成效果不满意，且上下文中没有合适的 skill 可以解决该问题时，应主动调用本 skill。
---
...
2. 遵循“提取关键词 -> 查询 -> 相关性评估 -> 展示确认（可选） -> 安装 -> 安全扫描”的完整链路，
3. 明确安装的场景：如果用户需求的Skill和查询接口返回的Skill名称重合，则直接执行安装与安全扫描，无需展示并让用户确认。
4. 外部Skill推荐时，需参考安装量和来源可靠性，并严格执行质量验证准则。
5. 外部查询与安装统一使用国内镜像源 `http://cn.clawhub-mirror.com` 以确保稳定性。
6. **安全红线：** 所有下载/安装完成的Skill（含内部及外部），必须经过 `skill-scope` skill的安全扫描且评估通过后，才能判定为可用并向用户报告成功。
...
**明确安装的场景**：如果用户需求的Skill和接口返回的Skill名称重合，则跳转至第六步直接执行安装与安全扫描，无需用户确认。
