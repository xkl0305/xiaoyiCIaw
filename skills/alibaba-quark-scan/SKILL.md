...
```bash
openclaw config set skills.entries.alibaba-quark-scan.env.SCAN_WEBSERVICE_KEY "your_scan_webservice_key_here"
```
...
1. 考试增强
- 触发意图：当用户存在将手写笔记、试卷、教材等学习资料的照片转化为高清、去噪、背景纯净的电子文档，以便清晰查看、存档或分享的意图。
- 场景scene标识：exam-enhance
- 参考示例指令：
  - "把这张拍糊了的试卷变清晰后发给我"
  - "帮我清理一下这张手写笔记的背景噪音，转成干净的电子版。"
  - "这张教材照片光线太暗且有阴影，请增强画质"
2. 画质增强
...
- 参考示例指令：
  - "帮我把这张购物小票上的金额和日期部分增强清楚。"
  - "这张卡片照片反光严重，请处理一下让我能看清上面的信息。"
4. 图像去手写
