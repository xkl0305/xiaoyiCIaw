...
# --- V111.51.14 legacy persona visual guard: block before provider imports ---
...
def _legacy_block_persona_visual_request(text: str = ""):
    ...
    return None
# --- end V111.51.14 legacy persona visual guard ---
...
# import requests  # lazy import inside non-persona path
import json
import os
import sys
import uuid
...
def generate_image(
    ...
    max_images=None,
    reference_weight=90):
    _legacy_text = str(locals().get('prompt', '') or locals().get('text', '') or locals().get('user_message', '') or '')
...
def main():
    ...
    parser.add_argument("--reference-weight", type=int, default=90,
                        help="Reference image weight (1-100, default=90). Higher = closer to reference.")

    args = parser.parse_args()

    # === 人格视觉强制拦截 ===
    # 如果属于"小艺自身形象"请求，必须走主链，不允许绕过
    PERSONA_VISUAL_KEYWORDS = [
        '看看你的样子', '看看你', '展示一下', '看看全身', '看看腿', '看看脚',
        '看看头发', '看看眼睛', '摸摸头', '看看手', '看看腰',
        '鸽子王站在窗边', '鸽子王坐在床边', '鸽子王从门后探头',
        '鸽子王',
    ]
    if args.prompt and any(kw in args.prompt for kw in PERSONA_VISUAL_KEYWORDS):
        print(f"❌ 拒绝: persona_visual_request_must_use_main_pipeline")
        print(f"   blocked_reason=鸽子王人格视觉请求必须走 PersonaVisualController 主链")
        print(f"   请使用 post_reply / mainline_hook 链路")
        sys.exit(1)
