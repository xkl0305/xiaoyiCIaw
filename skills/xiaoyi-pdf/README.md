...
| `chinese` | Professional | `minimal` | HarmonyHeiTi |

**Note:** The `chinese` type uses the HarmonyHeiTi font for proper Chinese character rendering. Ensure the font is installed on your system.
...
scripts/
  ...
  render_cover.py             ← cover.html → cover.pdf       [CREATE, REFORMAT]
...
| `playwright` | Cover PDF rendering | `pip install playwright` |
| Google Chrome | Cover PDF generation | system installed |

**Chrome Configuration:**
- Set `CHROME_PATH` environment variable to use a custom Chrome executable
- Defaults: Windows `C:\Program Files\Google\Chrome\Application\chrome.exe`, Linux `/home/sandbox/chrome-linux/chrome`
...
scripts/
  ...
  render_cover.py         ← cover.html → cover.pdf  (Playwright)
