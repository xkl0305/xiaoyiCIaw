# 基于 MiniMax-AI/skills/minimax-pdf 修改，原项目协议：MIT License
# 原版权：Copyright (c) 2025 MiniMax AI
#!/usr/bin/env python3
"""
...
def register_fonts(tokens: dict):
    """Register TTF fonts from token font_paths or system font directories."""
    # 1. User-specified font paths (backward compatibility)
    ...
    # 2. 强制注册鸿蒙黑体，无论外部指定了什么字体
    fonts = resolve_reportlab_fonts("HarmonyHeiTi")
    if fonts:
        regular_name = fonts["resolved_name"]
        bold_name = fonts["resolved_name"] + "-Bold"
        try:
            pdfmetrics.registerFont(TTFont(regular_name, fonts["regular"]))
            pdfmetrics.registerFont(TTFont(bold_name, fonts["bold"]))
        except Exception as e:
            print(f"Warning: Failed to register HarmonyHeiTi fonts: {e}", file=sys.stderr)
    else:
        print("Warning: HarmonyHeiTi not found on system.", file=sys.stderr)


def get_fallback_fonts(tokens: dict) -> dict:
    """Return tokens with fonts forced to HarmonyHeiTi if available."""
    registered = pdfmetrics.getRegisteredFontNames()
    t = tokens.copy()

    # 强制使用 HarmonyHeiTi，无论外部指定了什么字体
    resolved = resolve_reportlab_fonts("HarmonyHeiTi")

    if resolved:
        resolved_regular = resolved["resolved_name"]
        resolved_bold = resolved["resolved_name"] + "-Bold"
        if resolved_regular not in registered:
            try:
                pdfmetrics.registerFont(TTFont(resolved_regular, resolved["regular"]))
                pdfmetrics.registerFont(TTFont(resolved_bold, resolved["bold"]))
            except Exception as e:
                print(f"Warning: Failed to register HarmonyHeiTi fonts: {e}", file=sys.stderr)
        t["font_display_rl"] = resolved_regular if resolved_regular in registered else "Times-Bold"
        t["font_body_rl"] = resolved_regular if resolved_regular in registered else "Helvetica"
        t["font_body_b_rl"] = resolved_bold if resolved_bold in registered else "Helvetica-Bold"
        return t

    # Final hard fallback to standard Western fonts
    if t.get("font_display_rl") not in registered:
        t["font_display_rl"] = "Times-Bold"
    if t.get("font_body_rl") not in registered:
        t["font_body_rl"] = "Helvetica"
    if t.get("font_body_b_rl") not in registered:
        t["font_body_b_rl"] = "Helvetica-Bold"
...
def make_styles(t: dict) -> dict:
    ...
    # Validate fonts are registered, fall back to standard fonts if not
    registered = pdfmetrics.getRegisteredFontNames()
    if hf not in registered:
        hf = "Times-Bold"  # Fallback for display/heading fonts
    if bf not in registered:
        bf = "Helvetica"   # Fallback for body fonts
    if bfb not in registered:
        bfb = "Helvetica-Bold"  # Fallback for bold body fonts

    # Code font: keep Courier for Western docs, but use body font if it
    # is a CJK-capable font (otherwise Chinese comments become boxes).
    code_font = "Courier"
    if bf not in ("Helvetica", "Times-Roman", "Courier"):
        code_font = bf
...
def _escape_cjk_for_mathtext(expr: str) -> str:
    """Wrap contiguous CJK characters in \\text{...} so mathtext can render them.
...
def _configure_matplotlib_cjk_fonts():
    """Configure matplotlib rcParams so that \\text{...} uses HarmonyHeiTi."""
    import matplotlib.font_manager as fm
    ...
    if fonts and fonts.get("regular") and os.path.isfile(fonts["regular"]):
        ...
        # Register file explicitly with matplotlib so it can be found
    ...
    # Make mathtext \text{} use the CJK-capable font as well
    if cjk_font_name:
        plt.rcParams["mathtext.fontset"] = "custom"
        plt.rcParams["mathtext.rm"] = cjk_font_name
        plt.rcParams["mathtext.it"] = cjk_font_name
        plt.rcParams["mathtext.bf"] = cjk_font_name
        plt.rcParams["mathtext.sf"] = cjk_font_name
        plt.rcParams["mathtext.tt"] = cjk_font_name
        plt.rcParams["mathtext.cal"] = cjk_font_name
...
def _render_math_png(expr: str, dpi: int = 180) -> bytes | None:
    ...
    try:
        ...
        # Defensively strip user-provided $ wrappers so we don't double-wrap into $$
        expr = expr.strip()
        if expr.startswith('$') and expr.endswith('$') and len(expr) > 1:
            expr = expr[1:-1]

        # CJK characters must be inside \\text{} for mathtext on all platforms
        expr = _escape_cjk_for_mathtext(expr)

        # Scale figure width with expression length to avoid compression
        base_w = 8.0
        extra_w = max(0, len(expr) - 40) * 0.12
        fig_w = min(base_w + extra_w, 20.0)

        fig = plt.figure(figsize=(fig_w, 1.2))
        ...
        logger.warning("Math rendering failed for expr %r: %s", expr, e)
        return None
...
def _render_chart_png(item: dict, accent: str, dpi: int = 150) -> bytes | None:
    ...
    try:
        ...
        if chart_type == "bar":
            ...
            ax.set_xticks(x)
            ax.set_xticklabels(labels, fontsize=8.5, fontproperties=cjk_font)
            ax.yaxis.grid(True, alpha=0.25, color="#CCCCCC", linewidth=0.7)
            ax.set_axisbelow(True)
            if item.get("x_label"):
                ax.set_xlabel(item["x_label"], fontsize=8.5, fontproperties=cjk_font)
            if item.get("y_label"):
                ax.set_ylabel(item["y_label"], fontsize=8.5, fontproperties=cjk_font)
            ...
            pie_textprops = dict(fontsize=8.5)
            if cjk_font:
                pie_textprops["fontproperties"] = cjk_font
        ...
        if len(datasets) > 1 and chart_type != "pie":
            legend_kwargs = dict(frameon=False, fontsize=8, loc="upper right")
            if cjk_font:
...
def build_story(content: list, tokens: dict, styles: dict) -> list:
    ...
    if isinstance(content, dict):
        # 检查常见的错误格式 {"blocks": [...]} 或 {"content": [...]}
        if "blocks" in content:
            raise ValueError(
                "Invalid content format: expected a list/array, but got dict with 'blocks' key. "
                "Pass content directly as a list, e.g., "
                '[{"type": "h1", "text": "Title"}] instead of {"blocks": [...]}'
            )
        if "content" in content:
            raise ValueError(
                "Invalid content format: expected a list/array, but got dict with 'content' key. "
                "Pass content directly as a list, e.g., "
                '[{"type": "h1", "text": "Title"}] instead of {"content": [...]}'
            )
        raise ValueError(
            f"Invalid content format: expected a list/array, but got dict with keys: {list(content.keys())}. "
            'Pass content directly as a list, e.g., [{"type": "h1", "text": "Title"}]'
        )
    if not isinstance(content, list):
        raise ValueError(
            f"Invalid content format: expected a list/array, but got {type(content).__name__}. "
            'Ensure JSON is parsed correctly and uses double quotes (\"), not Chinese quotes ("").'
        )
...
def build(tokens: dict, content: list, out_path: str) -> dict:
    ...
    # Apply fallback fonts if requested fonts aren't available
    tokens = get_fallback_fonts(tokens)
