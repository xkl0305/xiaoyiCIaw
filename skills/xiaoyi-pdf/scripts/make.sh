...
#!/usr/bin/env bash
# make.sh — xiaoyi-pdf unified CLI
...
if not isinstance(data, list):
    print('ERROR: content.json must be a JSON array (list of blocks)', file=sys.stderr)
    sys.exit(1)
" || { red "Invalid JSON in $file"; exit 3; }
...
cmd_check() {
  ...
  if python3 -c "import playwright" 2>/dev/null; then
    ...
    yellow "  ⚠ playwright not installed  (run: make.sh fix)"
    ok=false
  fi
  ...
  if python3 -c "import PIL" 2>/dev/null; then
    green "  ✓ Pillow (image watermark support enabled)"
  else
    yellow "  ⚠ Pillow not installed — image watermarks will fail  (run: make.sh fix)"
    ok=false
  fi

  # pdfplumber
  if python3 -c "import pdfplumber" 2>/dev/null; then
    green "  ✓ pdfplumber (table extraction enabled)"
  else
    yellow "  ⚠ pdfplumber not installed — table extraction will fail  (run: make.sh fix)"
    ok=false
  fi

  # pandas
  if python3 -c "import pandas" 2>/dev/null; then
    green "  ✓ pandas (table data processing enabled)"
  else
    yellow "  ⚠ pandas not installed — table extraction will fail  (run: make.sh fix)"
    ok=false
  fi

  # openpyxl
  if python3 -c "import openpyxl" 2>/dev/null; then
    green "  ✓ openpyxl (Excel output enabled)"
  else
    yellow "  ⚠ openpyxl not installed — Excel output will fail  (run: make.sh fix)"
    ok=false
  fi
...
cmd_fix() {
  ...
  # Python packages
  if command -v python3 &>/dev/null; then
    python3 -m pip install --break-system-packages -q reportlab pypdf matplotlib playwright Pillow pdfplumber pandas openpyxl 2>/dev/null \
      || python3 -m pip install -q reportlab pypdf matplotlib playwright Pillow pdfplumber pandas openpyxl 2>/dev/null \
      || { yellow "  pip install failed — try: pip install reportlab pypdf matplotlib playwright Pillow pdfplumber pandas openpyxl"; rc=3; }
    green "  ✓ Python packages installed (reportlab, pypdf, matplotlib, playwright, Pillow, pdfplumber, pandas, openpyxl)"
  fi

  if [[ $rc -eq 0 ]]; then
    green "\nPython dependencies installed. Run: bash make.sh check"
  fi
  exit $rc
}
...
cmd_run() {
  ...
  save_mask=$(umask)
  umask 077
  workdir=$(mktemp -d "${TMPDIR}/XXXXXXXXXXXXXXXXXX")
  if [[ $? -ne 0 ]]; then
    umask "${save_mask}"
    red "Failed to create temporary directory"
    exit 1
  fi
  umask "${save_mask}"
...
JSON
  ...
  validate_content_json "$content_file"
...
cmd_fill() {
  ...
  # Write values to temp file to avoid shell quoting issues with JSON
  local val_file=""
  if [[ -n "$values" ]]; then
    val_file=$(mktemp "${TMPDIR:-/tmp}/minimax_values.XXXXXXXXXX")
    echo "$values" > "$val_file"
  fi

  local val_args=()
  if [[ -n "$val_file" ]];    then val_args+=(--data "$val_file"); fi
...
cmd_reformat() {
  ...
  save_mask=$(umask)
  umask 077
  tmpdir=$(mktemp -d "${TMPDIR}/XXXXXXXXXXXXXXXXXX")
  if [[ $? -ne 0 ]]; then
    umask "${save_mask}"
    red "Failed to create temporary directory"
    exit 1
  ...
  validate_content_json "$tmpdir/content.json"
...
[
  ...
  {"type":"body",    "text":"This document was generated by xiaoyi-pdf — a skill for creating visually polished PDFs. Every design decision is rooted in the document type and content, not a generic template."},
  ...
  {"type":"numbered","text":"render_cover.py uses Playwright to convert the HTML cover to PDF"},
...
JSON
  ...
  cmd_run \
    --title   "xiaoyi-pdf demo" \
    --type    "report" \
    --author  "xiaoyi-pdf skill" \
    --date    "$(date '+%B %Y')" \
...
# ── watermark ──────────────────────────────────────────────────────────────────
cmd_watermark() {
  PYTHONPATH="$SCRIPTS/..${PYTHONPATH:+:$PYTHONPATH}" $PY "$SCRIPTS/watermark_cli.py" "$@"
}

# ── merge ──────────────────────────────────────────────────────────────────────
cmd_merge() {
  PYTHONPATH="$SCRIPTS/..${PYTHONPATH:+:$PYTHONPATH}" $PY "$SCRIPTS/pdf_merge.py" "$@"
}

# ── split ──────────────────────────────────────────────────────────────────────
cmd_split() {
  PYTHONPATH="$SCRIPTS/..${PYTHONPATH:+:$PYTHONPATH}" $PY "$SCRIPTS/pdf_split.py" "$@"
}

# ── encrypt ────────────────────────────────────────────────────────────────────
cmd_encrypt() {
  PYTHONPATH="$SCRIPTS/..${PYTHONPATH:+:$PYTHONPATH}" $PY "$SCRIPTS/pdf_encrypt.py" "$@"
}

# ── decrypt ────────────────────────────────────────────────────────────────────
cmd_decrypt() {
  PYTHONPATH="$SCRIPTS/..${PYTHONPATH:+:$PYTHONPATH}" $PY "$SCRIPTS/pdf_decrypt.py" "$@"
}

# ── extract-text ───────────────────────────────────────────────────────────────
cmd_extract_text() {
  PYTHONPATH="$SCRIPTS/..${PYTHONPATH:+:$PYTHONPATH}" $PY "$SCRIPTS/pdf_extract_text.py" "$@"
}

# ── extract-tables ─────────────────────────────────────────────────────────────
cmd_extract_tables() {
  PYTHONPATH="$SCRIPTS/..${PYTHONPATH:+:$PYTHONPATH}" $PY "$SCRIPTS/pdf_extract_tables.py" "$@"
}

# ── dispatch ───────────────────────────────────────────────────────────────────
main() {
  if [[ $# -lt 1 ]]; then
    bold "xiaoyi-pdf — make.sh"
    echo ""
    ...
    echo "  watermark --input f.pdf --type text   WATERMARK: add text/image/pdf watermark"
    echo "  merge --inputs a.pdf b.pdf --out merged.pdf      Merge multiple PDFs"
    echo "  split --input doc.pdf --outdir dir/ [--ranges]   Split PDF into pages"
    echo "  encrypt --input doc.pdf --out e.pdf --password   Encrypt PDF with password"
    echo "  decrypt --input e.pdf --out d.pdf --password     Decrypt PDF with password"
    echo "  extract-text --input doc.pdf --out text.txt      Extract text to .txt"
    echo "  extract-tables --input doc.pdf --out tables.xlsx Extract tables to Excel/CSV"
    exit 0
  fi
  ...
  case "$1" in
    ...
    extract-text)   shift; cmd_extract_text   "$@" ;;
    extract-tables) shift; cmd_extract_tables "$@" ;;
