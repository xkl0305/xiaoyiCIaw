...
from font_resolver import get_css_font_stack


# ── Google Fonts loader ────────────────────────────────────────────────────────
def _gfonts_import(t: dict) -> str:
    """Return a CSS @import for Google Fonts, or @font-face for local fonts."""
    url = t.get("gfonts_import", "")
    if url:
        return f"@import url('{url}');"
    # 本地字体模式: 使用系统已安装的 HarmonyHeiTi
    return """
    @font-face {
        font-family: 'HarmonyHeiTi';
        src: local('HarmonyHeiTi');
        font-weight: normal;
    }
    @font-face {
        font-family: 'HarmonyHeiTi';
        src: local('HarmonyHeiTi-Bold');
        font-weight: bold;
    }
    """
...
html, body {{
    ...
    font-family: {get_css_font_stack(t['font_body'])};
}}
.page {{
    position: relative;
    width: 794px; height: 1123px;
