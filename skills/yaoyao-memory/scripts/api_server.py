...
# Memory单例（避免每次请求都创建新实例）
_memory_instance = None
_memory_cache_time = 0
_MEMORY_CACHE_TTL = 60  # 60秒内复用同一个实例

def _get_memory():
    """获取Memory单例"""
    global _memory_instance, _memory_cache_time
    import time
    
    if _memory_instance is None or (time.time() - _memory_cache_time) > _MEMORY_CACHE_TTL:
...
def _check_auth(handler) -> bool:
    ...
    try:
        sys.path.insert(0, str(MEMORY_SKILLS_DIR / "scripts"))
        from config_manager import has_config_password, verify_config_password
        if not has_config_password():
            return True
        auth = handler.headers.get("X-Password") or handler.headers.get("Authorization", "")
        if auth.startswith("Bearer "):
            auth = auth[7:]
        if auth and verify_config_password(auth):
            return True
    except Exception as e:
        print(f"Warning: {type(e).__name__}: {e}")
        pass
    handler.send_error(401, "Password required")
    return False
...
def get_stats(use_cache=True):
    ...
    try:
        ...
        if total < 10:
            ...
            health = 80
        # error 记忆是学习数据，仅在超过 30% 时才轻微惩罚
        error_rate = errors / max(total, 1)
        if error_rate > 0.30:
            penalty = int((error_rate - 0.30) * 100)
            health -= min(penalty, 10)  # 最多扣 10 分
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == "/api/memory_trends":
            self.send_json(run_memory_trends())
        elif path == "/api/memory_insights":
            self.send_json(run_memory_insights())
        elif path == "/api/memory_quality":
            self.send_json(run_memory_quality())
        elif path == "/api/smart_query":
            self.send_json(run_smart_query())
        elif path == "/api/query_predictor":
            self.send_json(run_query_predictor())
        elif path == "/api/snapshot_list":
            self.send_json(get_snapshot_list())
        elif path == "/api/hermes_status":
            self.send_json(get_hermes_status())
        else:
    ...
    def do_POST(self):
        ...
        handlers = {
            ...
            "set_config": lambda: (_check_auth(self), set_config_item)[1](data.get("key", ""), data.get("value", "")),
            ...
            "snapshot_create": lambda: create_snapshot(),
        }
...
# 新模块 API 包装函数
def run_memory_trends():
    try:
        sys.path.insert(0, str(MEMORY_SKILLS_DIR / "scripts"))
        from memory_trends import MemoryTrends
        mt = MemoryTrends()
        result = mt.calculate_health_score()
        mt.close()
        return result
    except Exception as e:
        return {"error": str(e)}

def run_memory_insights():
    try:
        sys.path.insert(0, str(MEMORY_SKILLS_DIR / "scripts"))
        from memory_insights import MemoryInsights
        mi = MemoryInsights()
        result = {"keywords": mi.extract_keywords(5), "topics": mi.identify_patterns()}
        mi.close()
        return result
    except Exception as e:
        return {"error": str(e)}

def run_memory_quality():
    try:
        sys.path.insert(0, str(MEMORY_SKILLS_DIR / "scripts"))
        from memory_quality import MemoryQualityAnalyzer
        mq = MemoryQualityAnalyzer()
        result = mq.get_quality_distribution()
        mq.close()
        return result
    except Exception as e:
        return {"error": str(e)}
...
def run_smart_query():
    try:
        sys.path.insert(0, str(MEMORY_SKILLS_DIR / "scripts"))
        from smart_query import SmartQueryRecommender
        sq = SmartQueryRecommender()
        result = {"suggestions": sq.get_time_based_suggestions()}
        sq.close()
        return result
    except Exception as e:
        return {"error": str(e)}

def run_query_predictor():
    try:
        sys.path.insert(0, str(MEMORY_SKILLS_DIR / "scripts"))
        from query_predictor import QueryPredictor
        qp = QueryPredictor()
        result = {"predictions": qp.get_combined_predictions(limit=5)}
        qp.close()
        return result
    except Exception as e:
        return {"error": str(e)}
...
def get_hermes_status():
    """Get Hermes Bridge integration status."""
    try:
        ...
        if HERMES_BRIDGE_PATH.exists():
            ...
            error_test = adapter.classify_error(429, "test")
            route_test = adapter.route_query("Hello")
            compress_test = adapter.compress([{"role": "user", "content": "Hi"}])
            
            return {
                "hermes_available": adapter.hermes_available,
                "error_classifier": error_test.get("reason") == "rate_limit",
                "smart_routing": route_test.get("use_cheap") == True,
                "context_compressor": True,
                "modules": [
                    "error_classifier", "smart_routing", "redact",
                    "rate_limit", "prompt_cache", "insights", "trajectory",
                    "context_compressor", "auxiliary_client",
                    "title_generator", "subdirectory_hints",
                ],
            }
        else:
            return {"hermes_available": False, "error": "hermes-bridge not found"}
    except Exception as e:
        return {"hermes_available": False, "error": str(e)}
