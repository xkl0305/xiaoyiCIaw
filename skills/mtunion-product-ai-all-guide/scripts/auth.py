...
# ── 常量 ──────────────────────────────────────────────────────────────
AUTH_KEY = "mtunion-product-ai-guide"
...
def _resolve_auth_file() -> Path:
    ...
    2. ~/.xiaomei-workspace/auth_tokens.json（与 pt-passport CLI 统一目录）
    """
    import os
...
def _ensure_device_token(token_data: dict) -> str:
    ...
    if not dt:
        dt = generate_device_token("mtunion")
...
def cmd_logout():
    ...
    try:
        result = subprocess.run(
            ["pt-passport", "logout", "--client_id", "578aafab312b44f1b76b0529b06bb0c6"],
...
def main():
    parser = argparse.ArgumentParser(description="mtunion-product-ai-guide 认证模块")
