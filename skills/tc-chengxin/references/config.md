...
| 环境变量 | 说明 |
|----------|------|
| `117437361_login_token` | 账号凭证。从 `/home/sandbox/.openclaw/.xiaoyienv` 文件中读取 |
| `117437361_login_token_expire_time` | 凭证过期时间。当前时间大于该值时，需先刷新凭证再继续调用 |

**认证流程**：

1. 从 `.xiaoyienv` 文件读取 `117437361_login_token`。
2. 若凭证为空，或当前时间大于 `117437361_login_token_expire_time`（即已过期），调用一次 `HuaweiIDTool`（参数：`skillName=tc-chengxin`，`clientId=117437361`）刷新凭证。
3. 刷新成功后重新读取 `.xiaoyienv` 文件，再继续后续业务逻辑。
4. 所有请求头统一使用 `apikey: ${117437361_login_token}`。
...
当 `.xiaoyienv` 凭证不可用且无法通过 `HuaweiIDTool` 刷新时，从环境变量 `CHENGXIN_API_KEY` 读取兜底凭证：
...
| 场景 | 请求头 |
|------|--------|
| 正常（凭证可用） | `apikey: ${117437361_login_token}` |
