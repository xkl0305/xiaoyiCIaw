...
- **预订/跳转链接仅含 app 链接**(`🔗 [app 预订](...)`),这是本渠道的预期行为,请原样输出,不要因为"缺少 PC/移动端链接"而补充或改写。
- 底部引导语
...
本渠道使用 **apikey** 请求头进行认证，主凭证来源为 `.xiaoyienv` 文件，兜底凭证来源为环境变量 `CHENGXIN_API_KEY`，**使用兜底凭证前必须先检查 .xiaoyienv 凭证，且必须已经调用过huawei_id_tool参数：clientId=117660163，skillName=tc-chengxin）工具且失败了**。
