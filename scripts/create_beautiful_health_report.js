...
async function generateCharts() {
  // 1. 睡眠趋势图
  const sleepCanvas = createCanvas(800, 400);
  ...
  new Chart(sleepCtx, {
    ...
    data: {
      ...
      datasets: [{
        label: '睡眠时长（小时）',
        data: [7.5, 6.8, 7.2, 7.0, 7.8, 7.5, 6.5],
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: '本周睡眠趋势',
          font: { size: 18, family: 'Microsoft YaHei', weight: 'bold' },
          color: COLORS.dark,
          padding: 20
        }
      },
  ...
  const sleepBuffer = sleepCanvas.toBuffer('image/png');
  fs.writeFileSync('/home/sandbox/.openclaw/workspace/skills/data-tracker/output/sleep_trend.png', sleepBuffer);
  
  // 2. 步数柱状图
  const stepsCanvas = createCanvas(800, 400);
  ...
  new Chart(stepsCtx, {
    ...
    data: {
      ...
      datasets: [{
        label: '步数',
        data: [8500, 7200, 12300, 9100, 8800, 5200, 7800],
        backgroundColor: [
          COLORS.chart1,
          COLORS.chart1,
          COLORS.success,
  ...
  const stepsBuffer = stepsCanvas.toBuffer('image/png');
  fs.writeFileSync('/home/sandbox/.openclaw/workspace/skills/data-tracker/output/steps_bar.png', stepsBuffer);
  
  // 3. 运动类型饼图
  const exerciseCanvas = createCanvas(600, 600);
  const exerciseCtx = exerciseCanvas.getContext('2d');
  
  new Chart(exerciseCtx, {
    type: 'doughnut',
    data: {
      labels: ['跑步', '游泳', '力量训练', '瑜伽'],
      datasets: [{
        data: [60, 45, 40, 30],
        backgroundColor: [
          COLORS.chart1,
  ...
  const exerciseBuffer = exerciseCanvas.toBuffer('image/png');
  fs.writeFileSync('/home/sandbox/.openclaw/workspace/skills/data-tracker/output/exercise_pie.png', exerciseBuffer);
  ...
  new Chart(radarCtx, {
    type: 'radar',
    data: {
      labels: ['睡眠质量', '运动表现', '日常活动', '营养摄入', '压力管理'],
      datasets: [{
        label: '本周',
        data: [85, 90, 75, 80, 70],
        borderColor: COLORS.primary,
  ...
  const radarBuffer = radarCanvas.toBuffer('image/png');
  fs.writeFileSync('/home/sandbox/.openclaw/workspace/skills/data-tracker/output/health_radar.png', radarBuffer);
  
  console.log('✅ 所有图表已生成');
}

// 创建文档
async function createBeautifulHealthReport() {
  // 先生成图表
                ...
                new TableRow({
                  children: [
                    ...
                    new TableCell({
                      ...
                      children: [
                        new Paragraph({
                          children: [new TextRun({ text: "2026年4月第3周", size: 18, color: COLORS.gray })],
...
// 主函数
async function main() {
  const doc = await createBeautifulHealthReport();
