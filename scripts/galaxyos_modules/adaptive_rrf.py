...
class AdaptiveRRF:
    ...
    def classify_query(self, query: str) -> QueryCategory:
        ...
        # 4. 分析查询特征
        words = re.findall(r'\w+', query)  # 支持中日韩等 Unicode 字符
        if not words:
            return QueryCategory.HYBRID
        
        # 判断是否中文为主（CJK 字符占比高）
        cjk_count = sum(1 for ch in query if '\u4e00' <= ch <= '\u9fff' or '\u3040' <= ch <= '\u30ff' or '\uac00' <= ch <= '\ud7af')
        is_cjk = cjk_count > len(query) * 0.3
        ...
        if is_cjk:
            ...
            # 含数字且短语 → 精确匹配
            if has_numbers and char_count < 20:
                return QueryCategory.EXACT_MATCH
            # 含特殊字符 → 关键词密集
