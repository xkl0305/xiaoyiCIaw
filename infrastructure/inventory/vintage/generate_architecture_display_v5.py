#!/usr/bin/env python3
"""
generate_architecture_display_v5.py - 生成架构展示页（6层逻辑优化版）

执行流程：用户请求 → 意图识别 → 技能匹配 → 执行 → 安全控制 → 结果返回

优化说明：
1. 技能独立为能力层（技能匹配环节）
2. 进化归入控制层（自我改进是控制的一部分）
3. 每层对应执行流程的一个环节
4. 文件数更均衡
"""
...
def render_main_display(inventory: dict) -> str:
    ...
    md = f"""# 鸽子王智能助手 - 主架构（6层逻辑优化版）

> **最近扫描时间**: {scan_time}
> **统计口径**: Markdown 文档数（排除 backup/repo/site-packages/reports/scripts）
> **架构版本**: 6层主架构 V5.0（逻辑优化版）

## 📊 执行流程

```
用户请求 → 意图识别 → 技能匹配 → 任务执行 → 安全控制 → 结果返回
    ↓          ↓          ↓          ↓          ↓          ↓
  入口      智能层      能力层     执行层     控制层     平台层
```
...
| 能力层 | {skills['md_count']} | 技能匹配 | 技能模块、可插拔能力 | skills |
| 执行层 | {layers['execution']['md_count']} | 任务执行 | 工作流、编排、自动化 | execution |
| 控制层 | {layers['control']['md_count']} | 安全控制 | 安全、合规、可靠性、进化 | control + resilience + evolution_lab |
